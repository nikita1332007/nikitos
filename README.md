...
Этот проект содержит функции для чтения данных из CSV и Excel файлов и преобразования их в удобный для использования формат — список словарей.
...
csv_result = read_csv(r'C:\path_to_your_file\transactions.csv')
print(csv_result)

xlsx_result = read_excel(r'C:\path_to_your_file\transactions_excel.xlsx')
print(xlsx_result)

# Банковские Транзакции

Этот проект предоставляет инструменты для работы с банковскими транзакциями. Он позволяет загружать данные из различных форматов файлов (JSON, CSV, XLSX), фильтровать и обрабатывать транзакции, а также анализировать их по категориям.

## Использование

1. **Запуск программы**: 
   Запустите основной скрипт, который содержит функцию main(). Она предоставляет пользователю интерфейс для выбора типа файла и дальнейшей обработки данных.

2. **Выбор файла**:
   Программа предложит выбрать тип файла для загрузки:
   - JSON
   - CSV
   - XLSX

3. **Фильтрация и сортировка**:
   После загрузки данных вы сможете фильтровать транзакции по статусу, сортировать их по дате и выполнять поиск по описанию.

4. **Анализ категорий**:
   Вы можете подсчитать количество транзакций по категориям, используя функцию count_transaction_types().

## Функции

### search_operations(transactions, search_str)

Ищет транзакции по описанию.

**Параметры**:
- transactions: Список транзакций (словарей).
- search_str: Строка для поиска в описании транзакций.

**Возвращает**: Список транзакций, соответствующих критериям поиска.

### count_transaction_types(transactions)
...
**Возвращает**: Словарь с количеством транзакций по категориям.
